              ...
              <div class="item-wrap" v-for="(item, index) in formJson.triggers" :key="index">
                ...
                <div v-if="item.thingsModel">
                  ...
                  <el-row :gutter="16">
                    ...
                    <el-col :span="10">
                      <el-select style="width: 100%" v-model="item.parentId" placeholder="请选择父级物模型" size="small" v-if="item.type == 1" @change="handleTriggerParentModelChange($event, index)">
                        <el-option v-for="(subItem, subIndex) in item.thingsModel.properties" :key="subIndex + 'triggerProperty'" :label="subItem.name" :value="subItem.regId"></el-option>
                      </el-select>
                      <el-select style="width: 100%" v-model="item.parentId" placeholder="请选择父级物模型" size="small" v-else-if="item.type == 2" @change="handleTriggerParentModelChange($event, index)">
                        <el-option v-for="(subItem, subIndex) in item.thingsModel.functions" :key="subIndex + 'triggerFunc'" :label="subItem.name" :value="subItem.regId"></el-option>
                      </el-select>
                      <el-select style="width: 100%" v-model="item.parentId" placeholder="请选择父级物模型" size="small" v-else-if="item.type == 3" @change="handleTriggerParentModelChange($event, index)">
                        <el-option v-for="(subItem, subIndex) in item.thingsModel.events" :key="subIndex + 'triggerEvents'" :label="subItem.name" :value="subItem.regId"></el-option>
                      </el-select>
                    </el-col>
                  </el-row>
              ...
              <div class="item-wrap" style="background-color: #eef3f7">
                <el-row :gutter="16">
                  ...
                  <el-col :span="8">
                    ...
                    <el-input v-model="form.executeDelay" size="small" prop="executeDelay" placeholder="秒钟" :max="90" :min="0" oninput="if(value>90)value=90;if(value<0)value=0" type="number" style="width: 140px">
                  ...
                  <el-row :gutter="16">
                    ...
                    <el-col :span="10">
                      <el-select style="width: 100%" v-model="item.parentId" placeholder="请选择父级物模型" v-if="item.type == 1" size="small" @change="handleActionParentModelChange($event, index)">
                        <el-option v-for="(subItem, subIndex) in item.thingsModel.properties" :key="subIndex + 'actionProperty'" :label="subItem.name" :value="subItem.regId"></el-option>
                      </el-select>
                      <el-select style="width: 100%" v-model="item.parentId" placeholder="请选择父级物模型" v-else-if="item.type == 2" size="small" @change="handleActionParentModelChange($event, index)">
                        <el-option v-for="(subItem, subIndex) in item.thingsModel.functions" :key="subIndex + 'actionFunc'" :label="subItem.name" :value="subItem.regId"></el-option>
                      </el-select>
                    </el-col>
                  </el-row>
  ...
  methods: {
    ...
    setParentAndModelData(sceneScript, sceneScripts) {
      for (let i = 0; i < sceneScripts.length; i++) {
        if (sceneScript.parentId == sceneScripts[i].regId) {
          ...
          if (sceneScript.parentModel.datatype.type === 'object') {
            ...
            for (let j = 0; j < sceneScript.parentModel.datatype.params.length; j++) {
              if (sceneScript.regId == sceneScript.parentModel.datatype.params[j].regId) {
    ...
    handleTriggerParentModelChange(identifier, index) {
      ...
      // 物模型赋值
      for (let i = 0; i < sceneScripts.length; i++) {
        console.warn(sceneScripts[i].regId);
        if (sceneScripts[i].regId == identifier) {
          ...
          if (sceneScripts[i].datatype.type === 'object') {
            ...
            // 普通类型，数组类
            this.formJson.triggers[index].id = sceneScripts[i].regId;
      ...
      for (let i = 0; i < sceneScripts.length; i++) {
        if (sceneScripts[i].regId == identifier) {
          ...
          if (sceneScripts[i].datatype.type === 'object') {
            ...
            // 普通类型，数组类
            this.formJson.actions[index].id = sceneScripts[i].regId;
